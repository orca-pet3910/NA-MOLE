                    ...
                    if ln[0] == "let":
                        ...
                        if ln[1] in self.variables:
                            if self.variables[ln[1]] == self._["nul"]:
                                self.onebyoneprint("nul")
                            elif self.variables[ln[1]] == self._["yea"]:
                                self.onebyoneprint("yea")
                            elif self.variables[ln[1]] == self._["nah"]:
                                self.onebyoneprint("nah")
                            else:
                                try:
                                    self.onebyoneprint(self.variables[ln[1]].replace(r"\n", "\n").replace(r"\r", "\r").replace(r"\"", "\""))
                                except:
                                    self.onebyoneprint(self.variables[ln[1]])
                        else:
                            if (ln[1][0] == '"' and ln[1][-1] == '"') or (ln[1][0] == "'" and ln[1][-1] == "'"):
                                if ln[1][0] == '"' and ln[1][-1] == '"':
                                    # if not '"' in ln[1][1:-1]:
                                        self.onebyoneprint(ln[1][1:-1])
                                    # else:
                        ...
                        print(self.onebyoneprint("""help for Now Actually Mighty Old Language Extended:
1. COMMANDS
...
2. VALUE TYPES
bool     : used for comparing/checking                 - possible values: yea, nah
_BOOL.nul: used when a value is not valid              - possible values: nul
str      : used for human-readable sentences and words - possible values: any unless malformed or "without quotes"
number   : used for mathematical/arythmetical
           expressions to represent decimal values     - possible values: any number without symbols and letters
3. ARGUMENTS
--nologo          : doesn't display the version and creator info
...
-p/--pause-on-exit: before MOLE exits, pause continued execution of the script (useful for self-closing terminals)"""))
                        ...
                        try:
                            ...
                            raise MOLEPackageCorrupted from MOLEPackageError(str("Possible package error. e.args[0]: " + e.args[0]))
                    elif ln[0] == "askbool":
                        if ln[1].strip() == "":
                            raise SyntaxError("you need to pass a variable in the first argument, otherwise the expression would be pointless")
                        else:
                            self.onebyoneprint(ln[2] + " [y/n] ")
                            input_value = input()
                        ...
                        if len(ln) == 4 and ln[1] in self.variables and ln[2] == "->":
                            try:
                                ...
                                raise MOLEPackageError(f"Cannot evaluate {ln[3]}, does the variable exist or contain a proper value?") from MOLEInterpreterError("A package error occured")
                        else:
                            raise MOLEPackageError("Cannot evaluate; did you forget to provide all arguments and symbols?") from SyntaxError("Malformed eval syntax")
                    elif ln[0] == "exit":
                        return
                    elif ln[0] == "asknumber":
                        try:
                            self.onebyoneprint(ln[2])
                        ...
                        if ln[2] == "->":
                            try:
                                self.variables[ln[3]] = read(ln[1])
                            except Exception as e:
                                raise MOLEInterpreterError from e
                        else:
                            raise SyntaxError("Malformed read syntax")
                    else:
